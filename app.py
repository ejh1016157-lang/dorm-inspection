...
# --- 메인 메뉴 ---
    ...
    st.sidebar.write(f"아이디: {st.session_state.user_id}")
    st.title("메인 메뉴")
...
# --- 입력 및 수정 페이지 ---
    ...
    for item in items:
        ...
        # 모바일 최적화: expander를 사용해 리스트를 접었다 펴기 가능하게 함
        with st.expander(f"📌 {item}"):
            if st.checkbox(f"{item} 점검 필요", key=f"chk_{item}", value=(val != "")):
    ...
    if st.button("제출"):
        ...
        if is_edit:
            df = df.drop(st.session_state.edit_idx)
...
# --- 이전 내역 페이지 ---
    ...
    if os.path.exists(FILE_NAME):
        ...
        # 모바일에서 데이터프레임이 잘 보이도록 일부만 표시하거나 스크롤 유도
        st.dataframe(df, use_container_width=True)
        idx = st.number_input("수정할 행 번호 선택", min_value=0, max_value=len(df)-1, step=1)
        if st.button("선택 내역 수정하기"):
            st.session_state.edit_idx = idx
            st.session_state.page = 'input'; st.rerun()
    else: st.write("저장된 내역이 없습니다.")
    if st.button("메인으로"): st.session_state.page = 'main_menu'; st.rerun()